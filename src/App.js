...
export default class App extends Component {
  ...
  connectWallet = async () => {
    ...
    try {
      ...
      await this.loadNfts();
        ...
        //             "10000000" + this.state.chainMeta.currencies[0].coinMinimalDenom
        //           );
    catch (err) {
      console.log(err)
      return
  ...
  //           await window.keplr.experimentalSuggestChain(this.state.chainMeta);
  //           await window.keplr.enable(this.state.chainMeta.chainId);
  //           let offlineSigner = await window.getOfflineSigner(
  //             this.state.chainMeta.chainId
  //           );

  //           // let gasPrice = GasPrice.fromString('10000000'+this.state.chainMeta.currencies[0].coinMinimalDenom);
  //           let gasPrice = GasPrice.fromString(
  //             "10000000" + this.state.chainMeta.currencies[0].coinMinimalDenom
  //           );
  ...
  ipfsUpload = async () => {
    ...
    reader.onload = async (event) => {
      ...
      try {
        let uploadResult = await IPFS.uploadFile(this.state.image);
        ...
        for (let i = 0; i < this.state.accounts.length; i++) {
          if (this.state.accounts[i]["address"]) {
            try {
              ...
              let balance = await this.state.cwClient.getBalance(
                  this.state.accounts[i].address,
                  this.state.chainMeta.currencies[0].coinMinimalDenom
              );
  ...
  getTokenMeta = async (tokenId = false) => {
    ...
    let ownerQuery = await this.state.queryHandler(
        this.state.contract,
        entrypoint
    );
    if (ownerQuery["owner"]) {
    ...
    console.log(
        "NFT contract succesfully queried for token ID " + tokenId,
        query
    );
  ...
  mintNft = async () => {
    ...
    let entrypoint = {
      mint: {
        token_id: String(this.state.nfts.tokens.length +1),
    ...
    try {
      ...
      let tx = await this.state.cwClient.execute(
          ...
          entrypoint,
          txFee
          // { amount: [{ amount: "500", denom: "ujunox" }], gas: "200000000" }
      );
  ...
  render() {
    ...
    const logItems = this.state.logs.length
              ...
              >
                <ul className="navbar-nav mr-auto">
                  <li className="nav-item">
                    <button
                        className={`btn ${
                            viewState === MARKET ? "btn-primary" : "btn-inverse"
                        }`}
                        onClick={() => this.changeDisplayState(MARKET)}
                    >
                      Market
                    </button>
                  </li>
                  <li className="nav-item">
                    <button
                        className={`btn ${
                            viewState === MINT ? "btn-primary" : "btn-inverse"
                        }`}
                  ...
                  <li className="nav-item">
                    <button
                        className={`btn ${
                            viewState === VIEW_OWNER ? "btn-primary" : "btn-inverse"
                        }`}
                        onClick={() => this.changeDisplayState(VIEW_OWNER)}
                    >
                      My NFTs
                    </button>
                  </li>
                ...
                <ul className="status accounts-list">
                  <li className="accounts account-item">
                    ...
                    <div>
                      ...
                      <span>
                      {parseInt(this.state.accounts[0].balance?.amount) /
                          100000 +
                          " " +
                          this.state.chainMeta.currencies[0].coinDenom}
                    </span>
                    </div>
                  </li>
                </ul>
              ...
              <div className="minting-form">
                ...
                <div className="image">
                  ...
                  >
                    ...
                    {/* Dropped files */}
                    <ul className="files-list-ul">{files}</ul>
                  </div>

                  <div className="controls minting-controls">
                    <button
                        className="btn btn-primary"
                        onClick={this.readyToMint}
                    >
                      Mint NFT
                    </button>
                  </div>
